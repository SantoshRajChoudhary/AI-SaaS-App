          ...
          {AiToolsData.map((tool, idx) => {
            const CardContent = (
              <>
                <tool.Icon
                  ...
                  aria-hidden="true"
                />
                <h3 className="mt-4 mb-1 font-semibold text-lg">
                  {tool.title}
                </h3>
                <p className="text-sm text-gray-500 max-w-[95%]">
                  {tool.description}
                </p>
              </>
            );
            ...
            return user ? (
              ...
              <Link
                key={idx}
                to={tool.path}
                className="p-8 flex flex-col gap-2 rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer max-w-xs focus:outline-none focus:ring-2 focus:ring-blue-400"
              >
                {CardContent}
              </Link>
            ) : (
              ...
              <div
                key={idx}
                onClick={() => navigate("/")}
                className="p-8 flex flex-col gap-2 rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 max-w-xs cursor-pointer opacity-90"
              >
                {CardContent}
              </div>
            );
