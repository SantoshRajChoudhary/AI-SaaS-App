          ...
          <div className='mt-2 text-xs flex flex-wrap gap-3'>
            {
              articlelength.map((item, index)=>(
                ...
                key={index} 
                className={`text-xs border rounded-full px-3 py-2 cursor-pointer ${selectedLength.text === item.text? 'bg-blue-50 text-blue-700 border-blue-300' : 'text-gray-500 border-gray-300'}`}>
                  {item.text}</span>
              ))
            }
          </div>
