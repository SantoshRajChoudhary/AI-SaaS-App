    ...
    return (
        <div className='px-4 sm:px-20 xl:px-32 py-24'>
            <div className='text-center'>
                <h2 className='mx-auto md:text-5xl sm:text-4xl text-3xl font-semibold text-slate-700'>Loved by Creators</h2>
                <p className='mt-4 sm:max-w-xl max-w-xs mx-auto text-sm sm:text-base text-gray-500'>Don't just take our word for it. Here's what our users are saying.</p>
            </div>
                    ...
                    <div key={index} className='p-8 m-4 max-w-xs rounded-lg bg-[#FDFDFE] shadow-lg border border-gray-100 hover:-translate-y-1 transition duration-300 cursor-pointer'>
                        <div className="flex items-center gap-1">
                           {Array(5).fill(0).map((_, index) => (
                            <img key={index} src={index < testimonial.rating? assets.star_icon: assets.star_dull_icon} className="w-4 h-4" />
                           ))}
                        </div>
